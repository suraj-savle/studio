...
export default function WillemScrollHeader() {
  ...
  const addToLoadingLetters = (el) => {
    if (el && !loadingLettersRef.current.includes(el)) loadingLettersRef.current.push(el);
  };
  const addToImageExtras = (el) => {
    if (el && !imageExtrasRef.current.includes(el)) imageExtrasRef.current.push(el);
  };
  const addToHeaderLetters = (el) => {
    if (el && !headerLettersRef.current.includes(el)) headerLettersRef.current.push(el);
  };
  const addToInteractiveLinks = (el) => {
    if (el && !interactiveLinksRef.current.includes(el)) interactiveLinksRef.current.push(el);
  };
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (boxRef.current) {
        tl.fromTo(boxRef.current, { width: "0em" }, { width: "1em", duration: 1.5 }, ">-0.2");
      }
      if (growingImageRef.current) {
        tl.fromTo(growingImageRef.current, { width: "0%" }, { width: "100%", duration: 1.5 }, "<");
      }
      if (headingStartRef.current) {
        tl.fromTo(headingStartRef.current, { x: "0em" }, { x: "-0.05em", duration: 1.5 }, "<");
      }
      if (headingEndRef.current) {
        tl.fromTo(headingEndRef.current, { x: "0em" }, { x: "0.05em", duration: 1.5 }, "<");
      }
      ...
      if (growingImageRef.current) {
        tl.to(growingImageRef.current, { width: "100vw", height: "100dvh", duration: 2 }, "+=0.2");
      }
      if (boxRef.current) {
      ...
      if (headerLettersRef.current.length) {
        tl.from(headerLettersRef.current, {
          yPercent: 100,
          duration: 1,
          stagger: 0.03,
        }, "<+=0.5");
      ...
      if (interactiveLinksRef.current.length) {
        tl.from(interactiveLinksRef.current, {
          yPercent: 100,
          duration: 1,
          stagger: 0.05,
        }, "<");
  ...
  return (
    <>
      ...
      <section
        ref={containerRef}
        className="invisible relative w-screen h-[100dvh] overflow-hidden text-[#f4f4f4] bg-[#f4f4f4] font-neue selection:bg-transparent"
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center text-[#201d1d] overflow-hidden pointer-events-none z-10">
          <div className="relative flex justify-center text-[5.5em] md:text-[9em] lg:text-[12.5em] font-medium leading-[0.75] whitespace-nowrap">
            ...
            <div ref={headingStartRef} className="flex justify-end w-[1.5256em] overflow-hidden">
              <span ref={addToLoadingLetters} className="relative block">W</span>
              <span ref={addToLoadingLetters} className="relative block">i</span>
              <span ref={addToLoadingLetters} className="relative block">l</span>
            </div>

            <div ref={boxRef} className="relative flex flex-col items-center justify-center w-0">
              <div className="relative flex items-center justify-center min-w-[1em] h-[95%]">
                <div ref={growingImageRef} className="absolute flex items-center justify-center w-0 h-full overflow-hidden">
            ...
            <div ref={headingEndRef} className="flex justify-start w-[1.525em] overflow-hidden">
              <span ref={addToLoadingLetters} className="relative block">l</span>
              <span ref={addToLoadingLetters} className="relative block">e</span>
              <span ref={addToLoadingLetters} className="relative block">m</span>
            </div>
        ...
        <div className="relative flex flex-col justify-between items-center w-full min-h-[100dvh] p-6 md:p-12 z-20 pointer-events-auto">
            ...
            <nav className="relative flex overflow-hidden w-full">
              <div className="flex justify-start items-start w-[55%] md:w-1/2">
                <div className="overflow-hidden">
                  <a ref={addToInteractiveLinks} href="https://www.osmo.supply" target="_blank" rel="noopener noreferrer" className="relative inline-block text-lg md:text-2xl leading-snug no-underline text-inherit">
                    Osmo ©
                  </a>
                </div>
              </div>
              ...
              <div className="flex flex-col md:flex-row justify-between items-start gap-6 md:gap-0 w-[45%] md:w-1/2">
                <div className="flex flex-col md:flex-row gap-2 md:gap-4">
                  <div className="overflow-hidden"><a ref={addToInteractiveLinks} href="#projects" className="relative inline-block text-lg md:text-2xl leading-snug no-underline text-inherit">Projects,</a></div>
                  <div className="overflow-hidden"><a ref={addToInteractiveLinks} href="#services" className="relative inline-block text-lg md:text-2xl leading-snug no-underline text-inherit">Services,</a></div>
                  <div className="overflow-hidden"><a ref={addToInteractiveLinks} href="#blog" className="relative inline-block text-lg md:text-2xl leading-snug no-underline text-inherit">Blog (13)</a></div>
                </div>
                <div className="flex overflow-hidden">
                  <a ref={addToInteractiveLinks} href="#touch" className="relative inline-block text-lg md:text-2xl leading-snug no-underline text-inherit">
                    Get in touch
                  </a>
                </div>
              </div>
          ...
          <div className="flex justify-between items-end w-full relative overflow-hidden">
            <div className="relative flex justify-center text-[5.5em] md:text-[9em] lg:text-[12.5em] font-medium leading-[0.75] whitespace-nowrap select-none">
              <span ref={addToHeaderLetters} className="relative block">W</span>
              <span ref={addToHeaderLetters} className="relative block">i</span>
              <span ref={addToHeaderLetters} className="relative block">l</span>
              <span ref={addToHeaderLetters} className="relative block">l</span>
              <span ref={addToHeaderLetters} className="relative block">e</span>
              <span ref={addToHeaderLetters} className="relative block">m</span>
              <span ref={addToHeaderLetters} className="relative block ml-[0.25em]">©</span>
            </div>
            
            <div className="hidden lg:block overflow-hidden">
              <p ref={addToInteractiveLinks} className="m-0 text-lg font-medium leading-snug text-white/60 pointer-events-auto text-center">
                Resource by{" "}
                <a href="https://www.osmo.supply" target="_blank" rel="noopener noreferrer" className="text-[#f4f4f4] no-underline">
                  Osmo
                </a>
              </p>
            </div>
      ...
      {/* Spacer Block to permit clean scroll room beneath the landing canvas */}
      <div className="w-full h-screen bg-[#201d1d] flex items-center p-12 text-white">
        <h2 className="text-4xl md:text-6xl font-medium font-neue">Next Section Content Rolls Up Smoothly...</h2>
      </div>
    </>
  );
}